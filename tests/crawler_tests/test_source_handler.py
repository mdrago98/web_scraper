...
from crawler.web_handlers.source_handler import ResourceDownloaderMixin, ResourceException
...
class TestResourceMixIn(TestCase):
    @patch('requests.get', lambda *args, **kwargs: MockResponse('loaded'.encode('utf-8'), 200))
    def test_fetch_resource_fetches_resource(self):
        sut = ResourceDownloaderMixin()
        result = sut.get_resource('', {})
        self.assertEqual(result.decode('utf-8'), 'loaded', 'Resource handler did not get the correct message')
